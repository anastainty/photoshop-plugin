...
Boolean DoUI(void)
{
    ...
    OSStatus err = initializeCocoaInvert();

    // Call function to show window
    // It's not an error: 1 = ok, 0 = cancel
    if (!err)
    {
        err = orderWindowFrontInvert();
    }
    else
    {
...
void DoAbout(void)
{
    ...
    [alert setMessageText:@"About Invert"];
    [alert setInformativeText:@"An example plug-in filter module for Adobe Photoshop."];
    [alert setAlertStyle:NSAlertStyleWarning]; // Исправлено на актуальный стиль
