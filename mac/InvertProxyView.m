...
extern InvertController *gInvertController;
...
*/
@implementation InvertProxyView
...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self) 
	{
		...
		NSLog(@"Invert SetColor to clearColor");
    }
    return self; 
}
...
{
	NSLog(@"Invert start setColor to %d", newColor);
	switch (newColor) 
	{
		case 0:
			currentColor = [NSColor clearColor];
			NSLog(@"Invert end setColor to clearColor");
			break;
		default:
		case 1:
			currentColor = [NSColor blueColor];
			NSLog(@"Invert end setColor to blueColor");
			break;
		case 2:
			currentColor = [NSColor redColor];
			NSLog(@"Invert end setColor to redColor");
			break;
		case 3:
			currentColor = [NSColor greenColor];
			NSLog(@"Invert end setColor to greenColor");
			break;
	}
}
...
- (void)viewDidChangeBackingProperties
	...
	*/
	...
	NSLog(@"Invert viewDidChangeBackingProperties, scale: %f", scaleFactor);
...
- (void)drawRect:(NSRect)rect 
{
	...
	if ( ! [NSThread isMainThread] ) 
	{
		...
		[gInvertController performSelectorOnMainThread:@selector(updateProxy:)
							   withObject:NULL
							waitUntilDone:NO];
		 */
	...
	NSLog(@"Invert drawRect, w: %f, h: %f, x: %f, y: %f, scale: %f", rect.size.width, rect.size.height, rect.origin.x, rect.origin.y, scaleFactor);
	...
	*/
	...
	short pixelDataHeight = (short) (drawRectPixelData.size.height);
	short pixelDataWidth = (short) (drawRectPixelData.size.width);
	...
	*/
	...
	short pixelDataDstRow = (short) (drawRectPixelData.origin.x);
	short pixelDataDstCol = (short) (drawRectPixelData.origin.y);
	...
	[gInvertController updateCursor];
}

@end

// end InvertProxyView.m
