...
//
//-------------------------------------------------------------------------------
DLLExport BOOL WINAPI InvertProc(HWND hDlg, 
								   UINT wMsg, 
								   WPARAM wParam, 
								   LPARAM lParam)
{
...
Boolean DoUI(void)
{
	...
	INT_PTR result = DialogBoxParam(GetDLLInstance(),
							...
							(DLGPROC)InvertProc,
							NULL);
	//used by the InvertProc routine, don't let the error go further
	*gResult = noErr;
	return (result == kDOK);
}
...
// UpdateProxyItem
//
// Force the WM_PAINT message to the InvertProc routine.
//
//-------------------------------------------------------------------------------
void UpdateProxyItem(HWND hDlg)
{
